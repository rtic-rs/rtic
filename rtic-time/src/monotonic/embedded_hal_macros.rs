    ...
    ($t:ty) => {
        impl $crate::embedded_hal::delay::DelayNs for $t {
            fn delay_ns(&mut self, ns: u32) {
                ...
                let mut done = now
                    + <Self as $crate::monotonic::Monotonic>::Duration::nanos_at_least(ns.into());
                if now != done {
                    // Compensate for sub-tick uncertainty
                    done = done + <Self as $crate::monotonic::Monotonic>::Duration::from_ticks(1);
                }
            ...
            fn delay_us(&mut self, us: u32) {
                let now = <Self as $crate::monotonic::Monotonic>::now();
                let mut done = now
                    + <Self as $crate::monotonic::Monotonic>::Duration::micros_at_least(us.into());
                if now != done {
                    // Compensate for sub-tick uncertainty
                    done = done + <Self as $crate::monotonic::Monotonic>::Duration::from_ticks(1);
                }
            ...
            fn delay_ms(&mut self, ms: u32) {
                let now = <Self as $crate::monotonic::Monotonic>::now();
                let mut done = now
                    + <Self as $crate::monotonic::Monotonic>::Duration::millis_at_least(ms.into());
                if now != done {
                    // Compensate for sub-tick uncertainty
                    done = done + <Self as $crate::monotonic::Monotonic>::Duration::from_ticks(1);
                }
    ...
    ($t:ty) => {
        impl $crate::embedded_hal_async::delay::DelayNs for $t {
            ...
            async fn delay_ns(&mut self, ns: u32) {
                <Self as $crate::monotonic::Monotonic>::delay(
                    <Self as $crate::monotonic::Monotonic>::Duration::nanos_at_least(ns.into()),
                )
                .await;
            }
