...
//! It is calculated to be 1 less than the lowest hardware task priority (to not have impact on
//! hardware tasks), but never lower than 1 above the highest software task priority (so the timer
//! interrupt can always preempt a software task), capped to the maximum priority in the system.
//! If no hardware task is available, it is set to the maximum priority in the system.
